...
if (!(await portIsAvailable())) {
  throw new Error(
    `Strict Stage 3G.1C review port ${port} is occupied.\n${portOwner()}`
  );
}
...
try {
  ...
  try {
    ...
    for (const [label, url] of Object.entries(links)) {
      ...
      if (label === 'authorityWaiverContract') {
        const summary = page.getByTestId('authority-contract-summary');
        if (!await summary.count()) {
          throw new Error('Authority contract summary is missing.');
        }
        const policyText = await page
          .getByTestId('authority-contract-policy')
          .textContent();
        if (!policyText?.includes('AUTHORITY-REQUIREMENT-POLICY-v1')) {
          throw new Error('Authority policy identity is missing.');
        }
        if (!policyText.includes('AUTHORITY-TRIGGER-POLICY-v1')) {
          throw new Error('Authority trigger policy identity is missing.');
        }
        if (!policyText.includes('١٠ محفزات نشطة')) {
          throw new Error('Typed active trigger count is not 10.');
        }
        if (!await workspace.getByText(
          'سلطة مطلوبة: لا يمكن إعفاؤها',
          { exact: true }
        ).count()) {
          throw new Error('Required-authority waiver warning is missing.');
        }
      }

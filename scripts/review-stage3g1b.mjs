...
if (!(await portIsAvailable())) {
  throw new Error(`Strict Stage 3G.1B review port ${port} is occupied.\n${portOwner()}`);
}
...
const links = {
  ...
  authorityContract: `${origin}/?workspace=readiness-pack&${scope}&readinessPackView=authorities`,
  ...
  try {
    ...
    for (const [label, url] of Object.entries(links)) {
      ...
      if (label === 'authorityContract') {
        ...
        if (!await summary.count()) throw new Error('Authority contract summary is missing.');
        if (!await summary.getByText('AUTHORITY-REQUIREMENT-POLICY-v1', { exact: true }).count()) {

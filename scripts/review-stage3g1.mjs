...
if (!(await portIsAvailable())) {
  throw new Error(`Strict Stage 3G.1 review port ${port} is occupied.\n${portOwner()}`);
}
...
const links = {
  ...
  sourceTraceability: `${origin}/?workspace=readiness-pack&${scope}&readinessPackView=sources`,
  workstreamMatrix: `${origin}/?workspace=readiness-pack&${scope}&readinessPackView=workstreams`,
  requirementAuthoring: `${origin}/?workspace=readiness-pack&${scope}&readinessPackView=requirements`,
  authorityMatrix: `${origin}/?workspace=readiness-pack&${scope}&readinessPackView=authorities`,
  evidenceContract: `${origin}/?workspace=readiness-pack&${scope}&readinessPackView=evidence`,
  spatialRelationships: `${origin}/?workspace=readiness-pack&${scope}&readinessPackView=spatial`,
  eligibilityAndFreeze: `${origin}/?workspace=readiness-pack&${scope}&readinessPackView=eligibility`,
  readinessCommand: `${origin}/?workspace=readiness&${scope}&readinessView=overview`
};
...
try {
  ...
  if ((await globalThis.fetch(`${origin}/local-assets/kap/kaga-zoning-candidate.jpg`)).status !== 200) {
    throw new Error('Local candidate preview failed.');
  }
  ...
  try {
    ...
    for (const [label, url] of Object.entries(links)) {
      ...
      if (url.includes('workspace=readiness-pack')) {

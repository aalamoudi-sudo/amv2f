...
if (!(await portIsAvailable())) {
  throw new Error(
    `Strict Stage 3G.1D review port ${port} is occupied.\n${portOwner()}`
  );
}
...
try {
  ...
  try {
    ...
    for (const [label, url] of Object.entries(links)) {
      ...
      if (label === 'trustedRoot') {
        const trust = page.getByTestId('readiness-pack-trust-status');
        if (!await trust.count()) {
          throw new Error('Trusted root status is missing.');
        }
        const trustText = await trust.textContent();
        if (
          !trustText?.includes('سلسلة الثقة المحلية مثبتة لهذه المراجعة')
          || !trustText.includes('R1')
          || !trustText.includes('سجل الأدلةغير متاح')
          || !trustText.includes('دفتر الإعفاءاتمثبت')
        ) {
          throw new Error('KAP local trust status changed unexpectedly.');
        }
      }

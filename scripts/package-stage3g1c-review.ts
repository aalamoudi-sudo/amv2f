...
const bundleName =
  'mayadeen-stage-3g1c-authority-waiver-trigger-integrity-review';
...
for (const resolution of resolutions) {
  ...
  const manifest = JSON.parse(readFileSync(manifestPath, 'utf8')) as {
    ...
    authorityTriggerPolicyId: string;
    screenshots: ScreenshotRecord[];
  };
  if (
    manifest.featureCommit !== currentCommit
    || manifest.packFingerprint !== packFingerprint
    || manifest.authorityPolicyId !== 'AUTHORITY-REQUIREMENT-POLICY-v1'
    || manifest.authorityTriggerPolicyId !== 'AUTHORITY-TRIGGER-POLICY-v1'
...
const stagingParent = mkdtempSync(
  path.join(os.tmpdir(), 'mayadeen-stage3g1c-review-')
);
...
try {
  ...
  const documentation = [
    'docs/stage-3g1c-authority-waiver-trigger-integrity-closure.md',
  ...
  const manifest = {
    stage: '3G.1C',
    status: 'READY_FOR_FOUNDER_STAGE_3G1C_REVIEW',
    ...
    authorityTriggerFingerprint: triggerFingerprint,
    sourceFingerprint:
    ...
    activeAuthorityTriggerCount: 10,
  ...
  writeFileSync(path.join(stagedRoot, 'README.md'), [
    '# Mayadeen Stage 3G.1C Founder Review',
    '',
    'This archive reviews the fail-closed authority-waiver and typed-trigger boundary.',
    '',
    'KAP has nine expected authority obligations and zero valid authority assignments.',
    'KAP remains candidate and operationally cannot-determine.',
    '',
    'No valid KAP waiver is represented. The synthetic positive path is unit-tested',
    'only because the browser has no legal KAP waiver evidence resolver.',
    '',

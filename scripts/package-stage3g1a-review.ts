...
const stagingParent = mkdtempSync(path.join(os.tmpdir(), 'mayadeen-stage3g1a-review-'));
...
try {
  ...
  const documentation = [
    'docs/stage-3g1a-final-readiness-pack-integrity-closure.md',
  ...
  writeFileSync(path.join(stagedRoot, 'README.md'), [
    '# Mayadeen Stage 3G.1A Founder Review',
    '',
    'This package reviews the final local readiness-pack integrity correction.',
    '',
    'The 61.7% measure is pack preparation only. KAP operational readiness remains cannot-determine.',
    '',
    'Raw PPTX, XLSX, DWG, video, contact data, GPS, credentials, and secrets are excluded.',
    ''
  ].join('\n'));
